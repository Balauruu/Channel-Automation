...
**Overall:** Multi-Agent Pipeline with Skill Injection
...
- **Memory persistence:** Each agent maintains a `MEMORY.md` file (`.claude/agent-memory/<agent>/MEMORY.md`) for cross-session learning, governed by the `agent-protocols` skill.
...
- Contains: 11 agent definitions (researcher, writer, strategy, editorial-lead, style-extractor, visual-researcher, visual-planner, asset-processor, asset-curator, compiler, code-reviewer)
...
- Contains: 22 skills spanning meta-behavior (agent-protocols, agent-observability, pipeline-design), domain expertise (autoresearch, visual-narrative, media-evaluation, archive-search, crawl4ai-scraping, data-analysis, structured-output), and user-invocable pipeline dispatchers (strategy, strategy-scrape, strategy-analyze, strategy-topics, compile, process-assets, write-script, visual-plan, assets-download, assets-embed, assets-search, assets-score)
...
- Contains: Three subdirectories organized by domain:
...
**Hooks Layer:**
- Purpose: Lifecycle hooks for observability, memory management, and cross-agent coordination
- Location: `.claude/hooks/`
- Contains:
  - `pipeline-observe.sh` -- Main observability hook (logs tool calls, durations, errors to `.claude/logs/`)
...
**Orchestration Layer:**
- Purpose: Cross-agent coordination signals and playbooks
- Location: `.claude/orchestration/`
- Contains: `PLAYBOOK.md` -- staging area for cross-agent handoff signals (authored by `@observer`, consumed by target agents)
- Depends on: Pipeline observer, agent-protocols skill
...
4. **Editorial Review** (`@editorial-lead`): Quality gate -- reviews dossier for source quality, claim verification, entity coverage, timeline consistency. Verbal feedback only (read-only tools). User decides whether to revise.
5. **Script Writing** (`@writer`): Transform research dossier into 3,000-7,000 word documentary script. Writes `Script.md` to `projects/<name>/script/`.
6. **Script Review** (`@editorial-lead`): Quality gate -- reviews script for voice compliance, fact-checking, narrative structure. Verbal feedback only.
7. **Visual Research** (`@visual-researcher`): Define visual intent per chapter, gather primary visual resources. Writes `visual_brief.json`, `media_leads.json`, downloads images to `projects/<name>/visuals/` and `projects/<name>/assets/`.
8. **Visual Planning** (`@visual-planner`): Generate structured shotlist with timing, b-roll curation, archive searches. Writes `shotlist.json` to `projects/<name>/visuals/`.
9. **Asset Processing** (`@asset-processor`): Download media, generate CLIP embeddings, run semantic search, score relevance. Writes to `projects/<name>/assets/`. User reviews and approves assets.
10. **Asset Curation** (`@asset-curator`): Deduplicate, evaluate reuse potential, promote assets to global library at `D:/Youtube/D. Mysteries Channel/3. Assets/`.
11. **Compilation** (`@compiler`): Compile edit sheet for DaVinci Resolve with timing-synced asset references. Writes `edit_sheet.md` and `manifest.json` to `projects/<name>/edit/`.

**State Management:**
- No shared runtime state -- all state lives in files on disk
- Agent memory (`.claude/agent-memory/<agent>/MEMORY.md`) persists cross-session learnings
- Project-scoped notes (`.claude/project-memories/<project>/<agent>.md`) persist per-project observations
- Skill insights (`.claude/skills/<skill>/insights.md`) accumulate learning from skill usage
- Cross-agent signals staged in `.claude/feedback/signals.yaml` (if exists) and `.claude/orchestration/PLAYBOOK.md`

**Cross-Agent Feedback Flow:**

1. Agent completes task, identifies cross-domain insight
2. Agent appends signal to `.claude/feedback/signals.yaml` (domain, type, promotion level)
3. Target domain agent reads signals at next task start (per `agent-protocols` skill)
4. Target agent resolves signal: promotes to MEMORY.md (if `promotion: memory`) or flags for review (if `promotion: definition`)

## Key Abstractions

**Agent Definition:**
- Purpose: A self-contained domain expert with identity, tools, procedures, and file conventions
- Examples: `.claude/agents/researcher.md`, `.claude/agents/writer.md`, `.claude/agents/strategy.md`
...
- Examples: `.claude/skills/agent-protocols/SKILL.md`, `.claude/skills/visual-narrative/SKILL.md`, `.claude/skills/autoresearch/SKILL.md`
...
- Pattern: Deterministic tasks (data extraction, counting, formatting) execute systematically. Heuristic tasks (narrative design, quality judgment) apply judgment. Prevents mechanical processing of creative tasks and vice versa.
...
- Responsibilities: Execute domain-specific tasks, produce artifacts, update memory

**User-to-Skill Dispatch (Pipeline Commands):**
- Location: `.claude/skills/*/SKILL.md` (invoked via `/skill-name` in Claude Code)
- Triggers: User command (e.g., `/strategy`, `/compile`, `/process-assets`)
...
- Responsibilities: Log observability data, check memory limits, coordinate cross-agent signals
...
**Logging/Observability:** Pipeline hooks in `.claude/hooks/pipeline-observe.sh` log every tool call (pre/post/fail/permission-denied) and subagent lifecycle to `.claude/logs/`. The `agent-observability` skill provides a structured schema for reading these logs. Summarizer at `.claude/scripts/obs-summarize.js` produces ~2-5KB markdown digests.
...
**Memory/Learning:** Three-tier persistent memory system:
1. Agent memory (`.claude/agent-memory/<agent>/MEMORY.md`) -- universal cross-project learnings, append-only, 200-line limit
2. Project notes (`.claude/project-memories/<project>/<agent>.md`) -- project-scoped observations, archived with project
3. Skill insights (`.claude/skills/<skill>/insights.md`) -- accumulated learnings per skill

**Quality Gates:** The `@editorial-lead` agent serves as the quality gatekeeper, reviewing research dossiers and scripts against structured rubrics. It has deliberately read-only tools (Read, Grep, Glob) and produces verbal feedback only.
