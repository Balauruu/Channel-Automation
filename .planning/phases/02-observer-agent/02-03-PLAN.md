...
must_haves:
  truths:
    ...
    - "Observer prompt explicitly overrides agent-protocols instructions for signals.yaml and project-memories/ to prevent conflict"
  artifacts:
...
<!-- agent-protocols conflict: observer must override these two instructions -->
agent-protocols instructs all agents to:
1. Write project-specific observations to .claude/project-memories/<project>/your-agent-name.md
2. (Implicitly reads signals.yaml if it exists via skill injection)
Observer must NOT do either of these -- it writes ONLY to ## Pending Review sections and rejections.jsonl.
The override must be explicit in observer.md body to prevent behavioral leakage.
</interfaces>
</context>
...
<task type="auto">
  ...
  <read_first>
    - .claude/skills/pipeline-design/SKILL.md (REQUIRED by CLAUDE.md before editing .claude/agents/ -- audit framework and 7 anti-patterns)
    ...
    - .claude/skills/agent-protocols/SKILL.md (protocols the observer inherits via skills injection -- note project-memories/ and signals.yaml instructions that must be overridden)
...
**IMPORTANT: Per CLAUDE.md, the pipeline-design skill must be consulted before creating agents. After reading pipeline-design/SKILL.md, apply the Decision Rule (Skill vs Agent-Body vs Bundled Reference) and the 7 anti-patterns audit mentally before writing. The observer is a single-consumer agent with tightly-coupled domain knowledge, so all content belongs in the agent body (not a skill or bundled reference).**
...
**Pipeline-design audit checklist (applied before writing):**
- Anti-pattern #1 (duplicated content): observer body is self-contained, no overlap with agent-protocols except the explicit override section
- Anti-pattern #2 (stale script references): no script paths -- observer uses only Claude Code tools
- Anti-pattern #3 (unused skills): agent-protocols is used (memory lifecycle at task start/end); no other skills needed
- Anti-pattern #5 (topic-specific biases): few-shot examples use 2 different topics (Duplessis orphans + generic writer), generalizable
- Anti-pattern #6 (CLAUDE.md thinking leaking): no [DETERMINISTIC]/[HEURISTIC] tags in observer body
- Anti-pattern #7 (dead routers): observer is not a router, it is a full processing agent
  </action>
  <verify>
    <automated>node -e "const fs=require('fs'); const f=fs.readFileSync('.claude/agents/observer.md','utf8'); const lines=f.split('\n').length; const checks=[['name: observer',f.includes('name: observer')],['model: sonnet',f.includes('model: sonnet')],['tools list',f.includes('- Read')&&f.includes('- Write')&&f.includes('- Edit')&&f.includes('- Bash')&&f.includes('- Grep')&&f.includes('- Glob')],['skills',f.includes('agent-protocols')],['memory: project',f.includes('memory: project')],['scope Q1',f.includes('Does this change how a specific skill')],['scope Q2',f.includes('Would a fresh instance of this agent')],['scope Q3',f.includes('Does this change how agents hand off')],['self-loop',f.includes('observer')&&f.includes('agent_id')],['8-run cap',f.includes('8 runs')],['cursor',f.includes('.observer-cursor')],['rejections',f.includes('rejections.jsonl')],['MEMORY format',f.includes('[HIGH]')&&f.includes('[MED]')&&f.includes('[LOW]')],['insights format',f.includes('from: researcher')],['min lines',lines>=200],['protocol override',f.includes('Protocol Override')],['no project-memories write',!f.includes('write to `project-memories')&&f.includes('Do NOT write to `project-memories')]]; let pass=0; for(const[n,ok]of checks){console.log(ok?'PASS':'FAIL',n);if(ok)pass++} console.log(pass+'/'+checks.length+' passed ('+lines+' lines)'); if(pass<checks.length)process.exit(1);"</automated>
  </verify>
  <done>
    ...
    - Body contains "## Protocol Overrides" section explicitly stating: do NOT write to project-memories/, do NOT read signals.yaml
    ...
    - No references to WebSearch, WebFetch, signals.yaml (as write target), project-memories/ (as write target), or @channel/channel.md
    - Pipeline-design skill audit was performed (documented in action section)
  </done>
</task>
